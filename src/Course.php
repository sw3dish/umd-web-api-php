...
class Course extends Method
{
	...
	/**
      ...
      *
      * If a query string is formatted : 
      * http://api.umd.io/v0/courses?<key>=<value>
      * then 
      * $params = array(
      *    "<key>" => array(
      *        "comparison" => "=",
      ...
      * "!=", "<", "<=", ">", ">="
      * then a query string reading : 
      * http://api.umd.io/v0/courses?<key>!=<value> becomes
      * $params = array(
      *   "<key>" => array(
      *        "comparison" => "!=",
      *        "value" => "<value>"
      ...
      * );
      *
      * If a query string supports comma-separated values,
      * add them to the "value" parameter as an indexed array. 
      ...
      * );
      *
      * If a query string supports the use of the pipe operator, add a key to
      * the array containing the comparison operator with the 
      * key "includeAllValues", with the value of true.
      * For example : http://api.umd.io/v0/courses?<key>=<value1>|<value2>
      * becomes : 
      * $params = array(
      *   "<key>" => array(
      ...
      * );
      *
      * @param array $params : Optional. An associative array parameters to be 
      * included in the query string. If no parameters are passed in, will 
      * return an array of courses for the current semester.
      * - string semester : Optional. a six-digit string <year><month>, defaults
      *   to semester currently on Testudo. 
      *   See http://api.umd.io/v0/courses?semester=012345 for
      *   a list of available semesters. Does not support multiple values. 
      *   Does not support non-standard comparison operators
      * - string page : Optional. Pagination is supported and is 1-based, 
      *   defaults to first page. 
      *   Does not support non-standard comparison operators.
      * - string per_page: Optional. Number of items per page, defaults to 30.
      *   Does not support non-standard comparison operators.
      *
      * @return array|object : The response body. Contains one or more objects
      * with course_id, name, and department.
	...
	/**
      ...
      *
      * @return array|object : The response body. Contains one or more Course
      * objects. Type is controlled by Request::setReturnAssoc().
      * More information on Course object can be found at 
      * http://umd.io/courses/#course_object
	 */
	...
	{	
		...
		$response = $this->request->api(
               'GET',
               '/v0/courses/sections?' . $queryString,
               array(),
               $headers
          );
	...
	public function getSections($sectionIds)
	{
		...
		$response = $this->request->api(
               'GET',
               '/v0/courses/sections/' . $sectionIds,
               array(),
               $headers
          );
	...
	/**
      ...
      *
      * @return array|object : The response body. Contains one or more 
      * department ids. Type is controlled by Request::setReturnAssoc().
